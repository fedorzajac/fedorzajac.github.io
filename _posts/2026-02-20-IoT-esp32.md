---
layout: post
title:  "🕸️ Internet of Things"
date:   2026-02-20 20:47:40 +0100
categories: mqtt micropython esp32
---

I bought microcontroller. Not my first one.

esp32 microcontroller with built in wifi, bluetooth, perfect for IoT.

To be able to use micropython, you need .bin file from

https://micropython.org/download/ESP32_GENERIC/
[https://micropython.org/resources/firmware/ESP32_GENERIC-20251209-v1.27.0.bin](https://micropython.org/resources/firmware/ESP32_GENERIC-20251209-v1.27.0.bin)


You connect esp32 with cable to computer
...
then, flash the microcontroller:
```bash
esptool.py --port /dev/cu.usbserial-110 erase_flash
# or
esptool.py --chip esp32 --port /dev/cu.usbserial-110 --baud 460800 write_flash -z 0x1000 esp32-*.bin
...
to be able to communicate, you will need
...
to view files on esp: `mpremote ls`
to remove file on esp: `mpremote rm file.py`
To blink a led on chip:
...
```
save it as `main.py` - the main.py will be run automaticaly
```bash
...
to co je? spoj to do jedneho postu a pridaj viac textovej omacky
---
layout: post
title:  "🕸️ Internet of Things: ESP32, MicroPython, and the Art of Making LEDs Blink"
date:   2026-02-20 20:47:40 +0100
categories: mqtt micropython esp32
---
...
But this time it’s **ESP32** — a tiny piece of silicon that somehow manages to squeeze in Wi-Fi, Bluetooth, enough GPIOs to get you into trouble, and just enough documentation to keep things interesting.
...
MicroPython gives you a Python 3–like environment running **directly on the microcontroller**. No OS, no containers, no systemd — just you, bare metal, and a REPL. It feels wrong in the best possible way.
...
Yes, it’s just a `.bin` file. No installer. No wizard. No safety net.
...
ls /dev/cu.*
```
You’re looking for something like usbserial, SLAB_USBtoUART, or similarly cryptic poetry.
...
pip3 install esptool
```
It’s a good idea to wipe the flash completely (clean slate, clean conscience):
```bash
esptool.py --port /dev/cu.usbserial-110 erase_flash
```
...
Useful commands you’ll need sooner than you think:
